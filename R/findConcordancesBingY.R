...
findConcordancesBingY <- function(exp=NULL, path_to_write, libName=NULL, userUploadedProfile=NULL, queryGenelist=NULL, write=FALSE,
...
# new cor code
            ...
            AllCorY <- data.frame(dataset=x_lib$signature_names[filtered],CC=cor_vec[filtered],PV=0,nGenes=vec$found_geneids)
            if (nrow(AllCorY)==0) { # no significant comparisons then return top 10
                ord <- head(order(abs(cor_vec), decreasing=TRUE),n=10)
                AllCorY <- data.frame(dataset=x_lib$signature_names[ord],CC=cor_vec[ord],PV=0,nGenes=vec$found_geneids)
            }
            rownames(AllCorY)<-AllCorY$dataset
            ord<-order(abs(AllCorY$CC),decreasing = T)
            AllCorY<-AllCorY[ord,]

		    # save(corTable, file="meh_corTable.RDS")
		    # save(AllCorY, file="meh_AllCorY.RDS")
		}
	}
