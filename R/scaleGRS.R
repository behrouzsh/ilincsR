...
scaleGRS <- function (query.p, reference.p, query.gL = NULL, reference.gL = NULL, 
    ...
    lambda = 0.005,rescale=T,plotRescaled=F) 
{
    ...
    if (!is.null(dim(query.p)) | !is.null(query.gL)) {
        ...
        pValue.query <- FALSE
        if (is.null(dim(reference.p))) 
            pValue.reference <- TRUE
        if (is.null(dim(query.p))) 
            pValue.query <- TRUE
        if (is.null(query.gL)) {
            if (tolerateWarnings) 
                warning("parameter query.gL should not be NULL")
            else return("parameter query.gL should not be NULL")
        }
        else {
            if (pValue.query) 
                NAindex <- is.na(query.p)
            else NAindex <- is.na(query.p[, 1])
            if (any(NAindex)) {
                if (na.rm) {
                  query.gL <- query.gL[!NAindex]
                  if (pValue.query) 
                    query.p <- query.p[!NAindex]
                  else query.p <- query.p[!NAindex, ]
                }
                else {
                  if (tolerateWarnings) 
                    warning("query.p has NA's but na.rm is FALSE")
            ...
            if (length(uniqueGenes) != length(query.gL)) {
                if (pValue.query) {
                  ...
                  averageScore <- unlist(lapply(averageScore, 
                    function(x) mean(as.numeric(x), na.rm = TRUE)))
                  averageProb <- unlist(lapply(averageProb, function(x) mean(as.numeric(x), 
                    na.rm = TRUE)))
                  query.gL <- uniqueGenes
                  query.p <- averageScore[match(query.gL, names(averageScore))]
                  query.p <- cbind(query.p, averageProb[match(query.gL, 
                    names(averageProb))])
        ...
        if (is.null(reference.gL)) {
            ...
            if (pValue.reference) 
                NAindex <- is.na(reference.p)
            else NAindex <- is.na(reference.p[, 1])
            if (any(NAindex)) {
                if (na.rm) {
                  reference.gL <- reference.gL[!NAindex]
                  if (pValue.reference) 
                    reference.p <- reference.p[!NAindex]
                  else reference.p <- reference.p[!NAindex, ]
                }
                else {
                  if (tolerateWarnings) 
                    warning("reference.p has NA's but na.rm is FALSE")
            ...
            if (length(uniqueGenes) != length(reference.gL)) {
                if (pValue.reference) {
                  ...
                  averageProb <- unlist(lapply(averageProb, function(x) mean(as.numeric(x), 
                    na.rm = TRUE)))
                  reference.gL <- uniqueGenes
                  reference.p <- averageScore[match(reference.gL, 
                    names(averageScore))]
                  reference.p <- cbind(reference.p, averageProb[match(reference.gL, 
                    names(averageProb))])
...
##	require(preprocessCore)
    ...
    if (estimateNullDistr) {
        ...
        f2 <- function(p1, p2, s1, s2) {
            index1 <- sample(length(p1))
            index2 <- sample(length(p2))
            RSCM(p1[index1], p2[index2], s1[index1], s2[index2])$p.value
        }
        geneTable <- geneTable[order(res$E.gene, decreasing = TRUE), 
            ]
        geneTable <- cbind(geneTable, Escore.pValue = rep(0, 
            dim(geneTable)[1]), Escore.qValue = rep(0, dim(geneTable)[1]))
        temp <- sapply(1:nullDistrN, function(i) list(f(p1, p2, 
            s1, s2)))
        temp2 <- sapply(1:nullDistrN, function(i) list(f2(p1, p2, 
            s1, s2)))
        ...
        for (i in 1:length(counts)) counts[i] <- counts[i] - 
            i
        geneTable[, 3] <- counts/(nullDistrN * length(counts))
        geneTable[which(geneTable[, 3] == 0), 3] <- 1/(dim(temp)[1] + 
            1)
        ...
        ret <- c(ret, list(resampledPvalues=temp2))
    }
    ret
}
