...
findConcordancesBing <- function(exp=NULL, path_to_write, libName=NULL, userUploadedProfile=NULL, queryGenelist=NULL, write=FALSE,
				   homoloGenes=TRUE, cutoff=0.05, logPcut=10, debuging=FALSE, sigDB="ilincs_sigs", algorithm="wtCor_all_exp", topSigs=FALSE, metadata=FALSE, chunk=500, ncors=10, org="Hs")
{
   ...
   if (libName %in% c('LIB_5','LIB_6','LIB_7') && algorithm == 'wtCor_all_exp') {
       return(findConcordancesBing(exp=exp, path_to_write=path_to_write, libName=libName, userUploadedProfile=userUploadedProfile, queryGenelist=queryGenelist, write=write, homoloGenes=homoloGenes, cutoff=cutoff, logPcut=logPcut, debuging=debuging, sigDB=sigDB, algorithm=algorithm, topSigs=topSigs, metadata=metadata, chunk=chunk, ncors=ncors, org=org))
   }
