...
findConcordancesBingZ <- function(exp=NULL, path_to_write, libName=NULL, userUploadedProfile=NULL, queryGenelist=NULL, write=FALSE,
...
if (libName=="LIB_3"){
	...
	if(algorithm %in% c("Cor_updn_exp")) {
	    refProfiles <- get(load(libraryPath))
	    libraryPvalues <- sub("2018/", "2018/signed_pLogs/", libraryPvalues)
	    libraryPvalues <- sub("PValues", "Plogs", libraryPvalues)
	...
	print(paste("LIB path: ", libraryPath))
	...
	if (libName=="LIB_3" & dim(queryProfile)[2] == 3){
	  ...
	  if(algorithm %in% c("wtCor_all_exp")) {
           # do not manipulate libraries (we didn't load them)
      } else {
		refData <- refProfiles[genes, ,drop=FALSE]
		no <- apply(refData,2,function(i) all(is.na(i)))
		refData <- refData[, !no ,drop=FALSE]
...
# * genes
            ...
            vec <- x_lib$read_df(queryData)

            if (vec$found_geneids == 0) {
              res$Remark <- "Filtered zero genes" # no overlapping genes between query and reference
              print("********************* Filtered zero genes!")
              return(res)
            }

            cor_vec <- x_lib$cor(vec,4)

            if (anyNA(cor_vec)) {
              res$Remark <- "Cor returns at least one NA -- bad library"
              print("********************* Cor returns at least one NA -- bad library!")
              return(res)
            }

            # converted to
            # rows: "LINCSCP_12356" "LINCSCP_12357" ...
            # cols: "signatureID" "estimate"  "pvalue" "df"
            # cols: "dataset", "CC", "PV", "nGenes"
            #
            # nGenes is based on sig library preprocessing rather than individial overlap (if missing weights -- this might differ)
            filtered <- which(abs(cor_vec)>=0.2)
            if (length(filtered) > 0) {
                 AllCorY <- data.frame(dataset=x_lib$signature_names[filtered],CC=cor_vec[filtered],nGenes=vec$found_geneids)
            } else { # no significant comparisons then return top 10
                ord <- head(order(abs(cor_vec), decreasing=TRUE),n=10)
                AllCorY <- data.frame(dataset=x_lib$signature_names[ord],CC=cor_vec[ord],nGenes=vec$found_geneids)
            }
            rownames(AllCorY)<-AllCorY$dataset
            ord<-order(abs(AllCorY$CC),decreasing = T)
            corTable<-AllCorY[ord,]

            # add PValues and filter
            pv<-2*pt(abs(sqrt(corTable$nGenes - 2) * corTable$CC / sqrt(1 - corTable$CC * corTable$CC)),corTable$nGenes-2, lower.tail=FALSE)
            corTable <- cbind(corTable, PV=pv)
            corTable <- corTable[corTable$PV <= cutoff,]
            corTable <- corTable[c("dataset","CC","PV","nGenes")]
		}
	}
