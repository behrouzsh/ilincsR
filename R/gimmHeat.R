                     ...
                     )
{
  ...
  scale01 <- function(x, low=min(x), high=max(x) )
    ...
    #gimmHeat
    if(length(gimmTrunc)==2){
	x[x<gimmTrunc[1] & !is.na(x)]<-gimmTrunc[1]
	x[x>gimmTrunc[2] & !is.na(x)]<-gimmTrunc[2]
    }
...
#print(length(col))
    if(length(breaks)==1)
      {
        gimmObsMax<-max(abs(unlist(x)), na.rm=na.rm)
#        breaks <- seq( min(x, na.rm=na.rm), max(x,na.rm=na.rm), length=breaks)
        breaks <- seq( -gimmObsMax, gimmObsMax, length=breaks)
      }
...
#          ...)
#   gimmObsMax<-max(abs(unlist(x)))
#   image(t(rbind(data.matrix(currData[,-(1:2)]),seq(-obsMax,obsMax,by=(2*obsMax)/(nCols-1)))),y=1:(nRows+1),ylim=c(1,nRows),col=pal)
#   print(dim(x))
#   print(nc)
#   print(nr)
#   print(gimmObsMax)
    image(1:(nc), 1:(nr+1), cbind(x,seq(-gimmObsMax,gimmObsMax,by=(2*gimmObsMax)/(nc-1))), xlim = 0.5+ c(0, nc), ylim = 0.5+ c(0, nr),
          axes = FALSE, xlab = "", ylab = "", col=col, breaks=breaks,
          ...)
