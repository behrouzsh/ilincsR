...
## Neither Bing or BingZ working after just installing from github
```{r}

install.packages("/Users/behrouzshamsaei/Documents/ilincsR", 
repos = NULL, 
type = "source")

library(ilincsR)
library(iLincsCor)
t2 <- findConcordancesBingZ(exp="sigFile.tsv",userUploadedProfile="/Users/medvedm/git/mario/ilincsTest/",path_to_write="/Users/medvedm/git/mario/ilincsTest/",metadata=T,libName="LIB_5",debuging=T, sigDB="ilincs_sigs",topSigs=F)
sapply(t2,class)
...
sig<-read.table(sigFile,header = T,sep="\t",stringsAsFactors = F)
sig2<-read.table("~/tmp/ilincs/tmp/processedSig_Fri_Oct_7_10_41_06_2022_5025179.xls",header = T,sep="\t",stringsAsFactors = F)

system.time(uploadJunk<-uploadSigToilincs(sig=sig,sigFilename=NULL,openWindow=F,genes=NULL,directory=".",deleteLocal=F,ilincsServer="http://www.ilincs.org/"))
system.time(junk1<-cmap(sig=NULL,ilincsFileName=uploadJunk["ilincsFileName"],ilincsLibrary="CP",directory=".",ilincsServer="http://www.ilincs.org/"))

#Not working 
system.time(uploadJunk<-uploadSigToilincs(sig=sig,sigFilename=NULL,openWindow=F,genes=NULL,directory=".",deleteLocal=F,ilincsServer="http://new.ilincs.org/"))
system.time(junk2<-cmap(sig=NULL,ilincsFileName=uploadJunk["ilincsFileName"],ilincsLibrary="CP",directory=".",ilincsServer="http://new.ilincs.org/"))

head(junk1)
head(junk2)
time curl 'http://new.ilincs.org/ocpu/library/ilincsR/R/findConcordancesBingZ' -X 'POST' -H 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8' --data 'libName="LIB_5"&exp="processedSig_Tue_Oct_11_13_58_01_2022_9138284.xls"&path_to_write="/mnt/raid/tmp/"&userUploadedProfile="/mnt/raid/tmp/"&metadata=F&logPcut=14'

# apiUrl="http://eh332new.ketl.uc.edu:8003/ocpu/library/ilincsR/R/findConcordancesBing/json/"
apiUrl="http://new.ilincs.org/ocpu/library/ilincsR/R/findConcordancesBingZ/json/"
req <- POST(apiUrl, body = list(exp="processedSig_Tue_Oct_11_13_58_01_2022_9138284.xls", userUploadedProfile="/mnt/raid/tmp/", path_to_write="/mnt/raid/tmp/", metadata=F, libName="LIB_5"),encode = "json")
res<-data.table::rbindlist(httr::content(req)$concordanceTable, use.names = TRUE, fill = TRUE)

##This is working - not sure where new.ilincs.org/ocpu is running - does not seem to have access to the newly uploaded file
system.time(uploadJunk<-uploadSigToilincs(sig=sig,sigFilename=NULL,openWindow=F,genes=NULL,directory=".",deleteLocal=F,ilincsServer="http://new.ilincs.org/"))
apiUrl="http://new.ilincs.org/ocpu/library/ilincsR/R/findConcordancesBingZ/json/"
system.time(res<-data.table::rbindlist(httr::content(POST(apiUrl, body = list(exp=uploadJunk["ilincsFileName"], userUploadedProfile="/mnt/raid/tmp/", path_to_write="/mnt/raid/tmp/", metadata=F, libName="LIB_5"),encode = "json"))$concordanceTable, use.names = TRUE, fill = TRUE))


##This is not working - not seing the ocpu server
apiUrl="http://www.ilincs.org/ocpu/library/ilincsR/R/findConcordancesBing/json/"
req <- POST(apiUrl, body = list(exp=uploadJunk["ilincsFileName"], userUploadedProfile="/mnt/raid/tmp/", path_to_write="/mnt/raid/tmp/", metadata=F, libName="LIB_5"),encode = "json")
res<-data.table::rbindlist(httr::content(req)$concordanceTable, use.names = TRUE, fill = TRUE)


apiUrl <- "http://new.ilincs.org/api/ilincsR/findConcordances"
req <- (POST(apiUrl, body = list(file=uploadJunk["ilincsFileName"], lib="LIB_5"), encode = "form"))
...
req <- POST(apiUrl, body = list(exp="sigFile2.tsv", userUploadedProfile="/mnt/raid/tmp/iLincs/signatures/", path_to_write="/mnt/raid/tmp/", metadata=T, libName="LIB_5", debuging=T, sigDB="ilincs_sigs", topSigs=F),encode = "json")
res<-data.table::rbindlist(httr::content(req)$concordanceTable, use.names = TRUE, fill = TRUE)
